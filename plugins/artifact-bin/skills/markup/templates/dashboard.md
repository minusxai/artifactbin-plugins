...
Beats: Header (kicker + one-sentence verdict + optional filter controls bound to Helmet Values) → KPI band (3-4 single-value tiles in one Grid row) → Evidence tiles (charts/tables on the same Grid) → Footnote (sources, window, refresh cadence)
...
- Header OUTSIDE the Grid: kicker `<p className="text-xs uppercase tracking-widest font-semibold text-primary">Ops · Weekly</p>`,
  ...
  drag"), then any shared filter controls in a row (`<div className="flex flex-wrap gap-3 mb-4">`):
  native `<select value="$region" options="$regions" />` / `<input type="range" value="$n" />`
  bound to `<Value>`s declared in `<Helmet>`, with every tile's `<Query>` binding `$region`
  in its SQL so the whole page follows one control.
...
TILE DISCIPLINE:
- Single-value tiles: `<Question data="$kpi" viz={{"kind":"single_value","yCols":["revenue"],"singleValueConfig":{"label":"Revenue","prefix":"$","format":",.0f"}}} />`
  over a one-row aggregate `<Query name="kpi">{`select sum(revenue) as revenue from ref_<id> where $region is null or region = $region`}</Query>`
  — the config's prefix/suffix/label/format does the talking; no caption divs above or
  inside tiles. (`singleValueConfig` written anywhere else renders a table.)
...
Do
- One Grid per section; live embeds only; a one-sentence verdict up top; shared filter
  Values for the whole page; reading-order GridItems.
Don't
